...
/**
 * Silhouette, split at the joints so limbs can rotate independently. Each part
 * is drawn once for the left side and mirrored, which is why one CSS
 * transform-origin serves both arms and both legs.
 */
const PARTS = {
  head:  '<ellipse cx="60" cy="17" rx="11.5" ry="13" fill="BODY"/><path d="M55,29 h10 v7 h-10 z" fill="BODY"/>',
  torso: '<path d="M60,36 C50,36 44,39 42,44 C38,54 37,66 39,78 L42,97 h18 V36 Z" fill="BODY"/>',
  arm:   '<path d="M41,44 C33,45 28,52 28,61 L30,86 C31,90 35,91 37,88 L40,62 Z" fill="BODY"/>',
  fore:  '<path d="M31,83 C29,94 28,108 29,118 C30,122 34,122 36,119 L39,85 Z" fill="BODY"/>',
  thigh: '<path d="M42,97 C40,104 40,110 42,116 L46,152 C47,157 53,157 55,152 L58,116 V97 Z" fill="BODY"/>',
  shin:  '<path d="M47,148 C45,160 45,176 47,188 C48,192 54,192 55,188 C57,176 57,160 56,148 Z" fill="BODY"/>',
};

/**
 * Movement archetypes. Not choreography per exercise — 150 of those would be
 * wrong more often than right — but the shape of the movement, which is what
 * you'd recognise across a gym anyway.
 */
const MOVEMENTS = [
  [/curl(?!.*leg)|chin-?up/,                              'curl'],
  [/pushdown|skullcrusher|tricep|close-?grip|kickback/,   'extension'],
  [/overhead press|shoulder press|military|arnold|push press|handstand/, 'press'],
  [/lateral raise|front raise|rear delt|fly|face pull/,   'raise'],
  [/row|pulldown|pull-?up|shrug/,                         'pull'],
  [/squat|leg press|lunge|split|step-?up|hip thrust/,     'squat'],
  [/deadlift|rdl|romanian|good morning|back extension|hyperextension/, 'hinge'],
  [/calf|heel raise/,                                     'calf'],
  [/crunch|sit-?up|leg raise|plank|rollout|russian twist/, 'crunch'],
  [/bench|chest press|push-?up|dip/,                      'benchpress'],
];

export function movementFor(exercise, name = '') {
  const label = `${exercise?.name ?? ''} ${name}`.toLowerCase();
  for (const [pattern, key] of MOVEMENTS) if (pattern.test(label)) return key;
  return null;
}
...
const MUSCLES = {
  // ---- front ----
  shoulders: {
    limb: 'torso', view: 'front', sided: true,
  ...
  // ---- back ----
  traps: {
    limb: 'torso', view: 'back', sided: false,
  ...
  lats: {
    limb: 'torso', view: 'back', sided: true,
  ...
  hamstrings: {
    limb: 'thigh', view: 'back', sided: true,
  ...
  calves: {
    limb: 'shin', view: 'back', sided: true,
...
function figure(view, litKey) {
  const forLimb = (limb) => Object.entries(MUSCLES)
    .filter(([, m]) => m.view === view && m.limb === limb)
    .map(([key, m]) => {
      const lit = key === litKey;
      const fill = lit ? COLOUR[key] : MUSCLE_REST;
      const group = `<g${lit ? ' class="mm-lit"' : ''}>${path(m.d, fill)}${fibres(m.f)}</g>`;
      // Muscles are mirrored inside their limb, so they travel with it.
      return m.sided ? both(group) : group;
    })
    .join('');

  const P = (key) => PARTS[key].replaceAll('BODY', BODY_FILL);

  // Nesting is the skeleton: forearm hangs off arm, shin off thigh. Rotating a
  // parent carries its children, which is what makes a curl look like a curl.
  const limb = both(`
    <g class="mm-arm">${P('arm')}
      <g class="mm-fore">${P('fore')}</g>
    </g>
    <g class="mm-thigh">${P('thigh')}
      <g class="mm-shin">${P('shin')}</g>
    </g>
  `);

  const armMuscles = both(`<g class="mm-arm">${forLimb('arm')}<g class="mm-fore">${forLimb('fore')}</g></g>`);
  const legMuscles = both(`<g class="mm-thigh">${forLimb('thigh')}<g class="mm-shin">${forLimb('shin')}</g></g>`);

  return `${P('head')}<g class="mm-torso">${both(P('torso'))}${forLimb('torso')}</g>${limb}${armMuscles}${legMuscles}`;
}
...
export function muscleMap(exercise, name = '', { both: pair = false } = {}) {
  ...
  if (!pair) {
    return `<svg class="${cls}" viewBox="0 0 120 200" role="img" aria-label="${label}" focusable="false">${
      figure(view, key)
    }</svg>`;
  }
